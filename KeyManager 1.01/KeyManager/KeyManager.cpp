...
string KeyManager::passwortTest(string& pfad,string& pwo) {
	...
	if (pwo.length() == 0) {
		...
		sha256 sha = sha256();
		sha.setMessage(mpw);
		unsigned char* hashpw = new unsigned char[32];
		hashpw = sha.getHash();
		string pwhashstring;
		pwhashstring.resize(16);
		for (int i = 0; i < 16; i++) {
			pwhashstring[i] = hashpw[i] ^ hashpw[i + 16];
		}
		mpw = pwhashstring;
...
void KeyManager::openDB(string pfado, string pwo) {
	...
	if (Cmode == 0) {
		...
		sha256 sha = sha256();
		sha.setMessage(txt);
		unsigned char * hashout = new unsigned char[32];
		hashout = sha.getHash();

		bool hashkorrekt = true;
		int d = 0;
		for (int i = 0; i < 4; i++) {
			if (tmp[tmp.size() - 2][i].char0 != hashout[d++] ||
				tmp[tmp.size() - 2][i].char1 != hashout[d++] ||
				tmp[tmp.size() - 2][i].char2 != hashout[d++] ||
				tmp[tmp.size() - 2][i].char3 != hashout[d++]) {
				hashkorrekt = false;
			}

		}
		for (int i = 0; i < 4; i++) {
			if (tmp[tmp.size() - 1][i].char0 != hashout[d++] ||
				tmp[tmp.size() - 1][i].char1 != hashout[d++] ||
				tmp[tmp.size() - 1][i].char2 != hashout[d++] ||
				tmp[tmp.size() - 1][i].char3 != hashout[d++]) {
				hashkorrekt = false;
			}
		...
		for (int i = 1; i < tmp.size()-2; i++) {
			...
			if(i==tmp.size()-3){		//wenn padding, dann im letzten block. alle anderen Blöcke sind ohne padding
				for (int j = 0; j < 4; j++) {
					if (out[j].byte0 == 0) {
						cou[0] = out[j].byte0;
						i = tmp.size();
...
int main() {
	...
	while (true) {
		...
		//cin >> text;
		text = "0000000000111111111122222222223333333333444444444455555555556666666666777777777788888888889999999999000000000011111111112222222222333333333344444444445555555555666666666677777777778888888888899999999990000000000";
		
		cout << endl;
		string hash = SHA_2_256(text);
		sha256 h = sha256();
		h.setMessage(text);
		...
		for (int i = 0; i< 32; i++) {
			if (hash2[i] >= 0x10) {
				printf("%x", (unsigned char)hash2[i]);
			}
			else {
				printf("%x", (unsigned char)hash2[i]);
			}
