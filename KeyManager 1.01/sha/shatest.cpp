...
*/
int hash1(int testno, int loopno, int hashno,
	...
	err = USHAReset(&sha, hashes[hashno].whichSha);
	if (err != shaSuccess) {
	...
	for (i = 0; i < repeatcount; ++i) {
		err = USHAInput(&sha, (const uint8_t *)testarray,
			length);
		if (err != shaSuccess) {
...
int hashfile(int hashno, const char *hashfilename, int bits,
	...
	if (bitcount > 0)
		err = USHAFinalBits(&sha, bits, bitcount);
	...
	*/
	for (hashno = hashnolow; hashno <= hashnohigh; ++hashno) {
		...
		for (loopno = 1; (loopno <= loopnohigh) && (err == shaSuccess);
			++loopno) {
			if (hashstr)
				err = hash1(0, loopno, hashno, hashstr, hashlen, 1,
				...
				for (testno = testnolow;
					...
					if (runHmacTests) {
						err = hash1(testno, loopno, hashno,
